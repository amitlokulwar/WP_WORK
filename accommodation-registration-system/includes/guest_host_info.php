...
<div class="leftguest">
          ...
          if($guestInfo[0]->country == $country && $guestInfo[0]->city== $city && $guestInfo[0]->state == $state 
          && ($_REQUEST['arrivalD'] !='' && $_REQUEST['departureD'] !='' && strtotime($_REQUEST['arrivalD']) >=strtotime($guestInfo[0]->arrival) && strtotime($_REQUEST['departureD']) <=strtotime($guestInfo[0]->departure)))
          {
                          $search1[] = $id->ID;
          }
					 }
         ...
         if(!$_REQUEST['search'] || $_REQUEST['find']== 1 || ($_REQUEST['find'] == 2 && $country == 0 && $city == 0 && $state == 0)){
                ...
                if($flag == 0 ){
                echo '<a href="'.home_url().'/guest-details?id='.base64_encode($allTerm->ID).'"><div class="hostProfile">';
                echo '<div class="profile"><img class="imgHost" src="'.$path['baseurl'].'/photo/'.get_user_meta($allTerm->ID,'media',TRUE).'"/></div>'; 
...
{
$cnt = 0;    
$guestInfo = $o->getHostInfo($id->ID);
 if(user_can($id->ID,'host'))
 {
     if(!empty($guestInfo) && get_user_meta($id->ID,'country',TRUE)== $country && get_user_meta($id->ID,'city',TRUE)== $city && get_user_meta($id->ID,'state',TRUE)== $state)
     { 
       foreach ($guestInfo as $guest){
       if($_REQUEST['arrivalD'] !='' && $_REQUEST['departureD'] !='' && strtotime($_REQUEST['arrivalD']) >=strtotime($guest->dateFrom) && strtotime($_REQUEST['departureD']) <=strtotime($guest->dateTo)){
           ++$cnt;
       }  
       }
       if($cnt == 0){$search[] = $id->ID;}
       
     }
     else if(get_user_meta($id->ID,'country',TRUE)== $country && get_user_meta($id->ID,'city',TRUE)== $city && get_user_meta($id->ID,'state',TRUE)== $state)
          {
                  $search[] = $id->ID;
          }
 }
  ...
  if(empty($host['include']))
  {
        $flag2 = 1;    
          ...
          foreach ($myrows as $value) {
             if($flag2 == 0){
             echo '<a href="'.home_url().'/host-details?id='.base64_encode($value->ID).'"><div class="hostProfile">';
             echo '<div class="profile"><img class="imgHost" src="'.$path['baseurl'].'/photo/'.get_user_meta($value->ID,'profile',TRUE).'"/></div>'; 
